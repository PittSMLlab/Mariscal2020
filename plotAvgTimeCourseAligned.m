function varargout=plotAvgTimeCourseAligned(adaptDataList,params,conditions,binwidth,trialMarkerFlag,indivFlag,indivSubs,colorOrder,biofeedback,removeBiasFlag,labels,medianFlag,plotHandles,alignEnd,alignIni,ah,medfilter)
...
%         end
        ...
        for c=1:nConds
             ...
             %%Dulce  TURNED OFFF 3/6/2019
            if  strcmp(cond{c},{'Gradualadaptation'})
              alignEnd2=alignEnd; 
              alignEnd=[];
            else
                alignEnd=alignEnd2;
...
%                     else
                    values(group).(params{p}).(cond{c}).(['trial' num2str(t)])(subject,1:nPoints)=[dataPts(:,p)];
%                        values(group).(params{p}).(cond{c}).(['trial' num2str(t)])(subject,1:size([dataPts(:,p); nan(20,1)]),1)=[dataPts(:,p); nan(20,1)];
%                     end
                    if ~isempty(alignEnd) %Aligning data to the end too, by creating a fake condition
                        values(group).(params{p}).([cond{c} 'End']).(['trial' num2str(t)])(subject,:)=[nan(20,1); dataPts(end-alignEnd+1:end,p)];
                    end
...
%%Activate to plot after effects 
% if ~isempty(alignEnd) %PABLOS CODE  
...
%%Activate to plot gradual adaptation 
for c=1:nConds %Modification to plot entire adaptation and other condition by part 
    if  strcmp(cond{c},{'Gradualadaptation'}) || isempty(alignEnd)
        newCond{c}=cond{c};
    else
        newCond{c}=cond{c};
        newCond{c+1}=strcat(cond{c},'End');
        cond=newCond;
        nConds=length(cond);
    end
end
...
%                 end
            ...
            if strcmp(cond{c},'Gradualadaptation')
                [maxPts,loc]=nanmax(numPts.(cond{c}).(['trial' num2str(t)]));
                maxPts=1000;
            elseif strcmp(cond{c},'readaptation')
                [maxPts,loc]=nanmax(numPts.(cond{c}).(['trial' num2str(t)]));
                maxPts=140;
            elseif strcmp(cond{c},'OG base') || strcmp(cond{c},'TM base')
                [maxPts,loc]=nanmax(numPts.(cond{c}).(['trial' num2str(t)]));
                maxPts=100;
            else
                [maxPts,loc]=nanmax(numPts.(cond{c}).(['trial' num2str(t)]));
            ...
            for p=1:length(params)                
                ...
                if strcmp(cond{c},'readaptationEnd') %ADDED 3/8/2019 error on the maxPTs 
                 maxPts=40;
                end
                
                if ~isnan(maxPts)%do not try to plot if maxPts is NaN, this indicates that there is no c3d for this trial
%                     allValues=values(group).(params{p}).(cond{c}).(['trial'    num2str(t)])(:,1:maxPts-5); %TURN ON FOR ADAPTATION DULCE
                     allValues=values(group).(params{p}).(cond{c}).(['trial' num2str(t)])(:,1:length(values(group).(params{p}).(cond{c}).(['trial' num2str(t)]))-5);
                else
...
%                 stop = start+(binwidth-1);
                ...
                hold on
                afterTrialPad=10; %adds empty sapce to end of trial/condition (in strides)
...
%                 condLength=length(y);
                if ~isempty(alignIni) && strcmp(cond{c}(end-2:end),'End') ||  ~isempty(alignIni) && strcmp(cond{c},'Gradualadaptation') ||  isempty(alignIni) 
                      
                    condLength=length(y);
                    x=Xstart:Xstart+condLength-1;

                elseif  ~isempty(alignIni) && ~strcmp(cond{c},'Gradualadaptation')|| ~isempty(alignIni)
                    %                     if  strcmp(cond{c},{'Gradualadaptation'})
                    %                         Ystart=250;
                    %                         alignIni2=150;
                    %                     else
                    ...
                    y=y(Ystart:Ystart+length(x)-1);
                    E=E(Ystart:Ystart+length(x)-1);
                    condLength=length(y);
                    

                end
                if strcmp(cond{c},'Gradualadaptation')|| strcmp(cond{c},'readaptation')
                x=downsample(x,5);
                y=downsample(y,5);
                E=downsample(E,5);
                end
                ...
                if indivFlag %plotting all individual subjects
                    ...
                    %in the group
                    if length(adaptDataList{group})>1
                        Li{group}(length(subsToPlot)+1)=plot(x,y,'.','MarkerSize',5,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[0.7 0.7 0.7].^group);                    
                ...
                elseif  isempty(biofeedback) %Each group colored differently
                        ...
                        %%
                        %Dulce 3/7/2019 TURN OFF this section plot the last strides
                        %as dot for post adaptation data
%                         if c==2 &&  strcmp(cond{c}(end-2:end),'End') && ~strcmp(cond{c},'GradualadaptationEnd') && ~strcmp(cond{c},'readaptationEnd')
%                             plot(x(end-2), nanmean(y),'.','MarkerSize',5,'LineWidth',0.02,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',color,'Color', color)%CJS
%                             
%                             hold on
%                             errorbar(x(end-2), nanmean(y), nanmean(E), 'Color', color)
%                         else
...
%                         end
        ...
        if c==nConds && group==Ngroups
            ...
            for p=1:length(params)
                if ~(size(params,1)>1)
                    axes(ah(p))
%                      ylabel(params{p},'fontsize',titleFontSize)
                     ylabel(params{p}(1:end-17))
                else
                    axes(ah)
                end
                axis tight
                ...
                if isempty(alignEnd)
                    ...
                    line([lineX(1:2:end); lineX(1:2:end)],ylim,'color','k')
                     ...
                     line([lineX(2:2:end); lineX(2:2:end)],ylim,'color','k')   
                    end
                end
...
labels={labels}';
 legend([Li{:}],[labels{:}],'Location','northeast')
end
%% outputs
if nargout<2
